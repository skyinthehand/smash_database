...
Alternatives参照)。参加者数比較に基づく確定(FR-005の恒久ロック)は
「取得処理(クロール実行/ツール実行)をまたいだ」場合にのみ適用し、
同一の取得処理内でまだ確定していない側同士は、3件以上でも常に最新の
参加者数で再比較する(取得処理開始時点でスナップショットする
`settled_tournament_ids`により判定。`research.md` Decision 3、ユーザー
フィードバック2026-08-29)。
