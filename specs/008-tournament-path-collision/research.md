...
**実装後・実データ検証での訂正(2026-08-29)**: `find_path_collisions.py`を
実際の`data/startgg/tournaments.jsonl`(12件の衝突)に対して実行したところ、
**全件で、衝突する2つのevent_idの登録`path`が文字列として完全に同一**である
ことが判明した(=衝突とは、常に物理的に1つのディレクトリしか存在しない状態を
指す。後から取得された側が無条件に上書きするため、もう一方の実データは既に
消滅している)。当初の`Target.load_attr()`は、両ターゲットの`current_path`
(=同一)からそれぞれ`attr.json`を読み込み、参加者数を比較していたが、実際には
両者とも**同じファイル**を読み込むことになり、生存している方のevent_idを検証
しないまま比較・tie-breakを行っていた。この結果、どちらが「勝者」になっても
`move_event_dir()`は2番目に処理された側の実ディレクトリ(=生存データそのもの)
を敗者側の調整名パスへ実際に移動してしまい、勝者側の登録は空になったディレクト
リを指す、という**12件全てで実データを破壊する**バグがあった(ユニットテストは
2つの別々の物理ディレクトリを用意する合成フィクスチャで検証していたため、この
実際の構造を再現できておらず見逃されていた)。

**さらなる訂正(2026-08-30、ユーザーフィードバック)**: 上記の`owns_data`
(`attr.json`内の`event_id`と自分自身が一致するか)による判定にも、同様の
問題が残っていた。`matches.json`の逐次取得が未完了のまま中断した場合
(still_incomplete)、`write_event_attributes()`が呼ばれず`standings.json`
だけが新しいイベントのもので上書きされ、`attr.json`だけ古いイベントの情報が
残る、という状態になり得る。つまり`attr.json`の`event_id`もまた、その時点で
実際にディレクトリに入っているデータの持ち主を保証しない。**ローカルファイル
だけでは「どちらのevent_idのデータか」を確実には判定できない**という結論に
至ったため、最終的に以下の設計に変更した:

- 各対象event_idについて、`redownload_event.py`と同じ`fetch_event_details()`
  を使ってstart.ggから素直な(衝突しうる)保存先パスを算出した上で、
  `download_standings()`をstart.ggへ直接実行し直し、その場でのみ得られる
  真の参加者数を取得する(ローカルの`attr.json`/`num_entrants`は一切信頼
  しない)。
- 参加者数が最多の1件(同数ならtournament_idが小さい方)を無条件に勝者とし、
  `redownload_event.py`と全く同じ手順(`download_standings`→`download_seeds`
  →`download_all_set`→`write_event_attributes`)で改めて完全に再取得し、
  素直な(衝突しうる)保存先パスへ書き込む(既存の内容は破棄する)。
- 敗者側は、再取得した真の参加者数が**必ず0であることを要求する**安全確認
  を行う。0でない敗者が1件でもあれば、単純なテスト大会ではない可能性がある
  として自動実行(`--yes`)を中止し、手動確認を促す。0であることを確認できた
  敗者のみ、`tournaments.jsonl`の登録を削除する(ディレクトリ操作は発生
  しない。敗者側に実データは無いため)。
- start.gg側で対象イベント自体が既に見つからない(削除済み)場合は、
  参加者数0・実データなしとして扱う(それ自体が実質的な確認材料になる)。

この設計により、`--token`(start.ggへの実アクセス)が再び必要になったが、
「ローカルファイルの状態を信頼する」という根本的に不確実な前提を排除できた。
