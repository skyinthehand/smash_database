...
**2つの取得経路、1つのスキーマ。** 既存の一括ページング取得
（`event.sets`/`phaseGroup.sets`）は引き続き主経路であり、`matches.json`が
まだ存在しないイベントに対しては常に最初に試みられる（research.md §0）。
これが成功した場合、全レコードは以下の`complete`状態で直接書き込まれる——
プレースホルダーは一切現れず、追加のクエリも発生しない（`set_id`は一括
クエリの既存の`id`フィールドから無償で得られる）。一括取得が失敗した場合
にのみ、そのイベントはresearch.md §1〜§4で述べる逐次取得フォールバック
経路に入り、レコードはまず`placeholder`状態を経由する。どちらの経路も
同じファイル（`matches.json`）に、以下の同じ2つのレコード形状で書き込む
——スキーマ自体は、ある`complete`レコードがどちらの経路で生成されたかを
区別しない。
...
```
（存在しない） --[一括取得に成功]-------------------> complete（プレースホルダーを経由しない）
（存在しない） --[一括取得が失敗し、set ID一覧を取得]--> placeholder
...
`set_id`が変わることも無い（FR-005, FR-009）。1つのイベント内で1つの`set_id`
につき常にちょうど1件のレコードが存在する（FR-009）——既にレコードが存在する
`set_id`（プレースホルダーであれ完了済みであれ）に対して、取得処理が2件目の
レコードを追記することは無い。

### バリデーションルール（spec.mdのFunctional Requirementsから導出）

- FR-003/FR-008: イベントについて（ID一覧取得により）判明している全ての
  `set_id`は、`matches.json`内に対応するレコード（プレースホルダーまたは
  完了済み）をちょうど1件持たなければならない。
- FR-009: 同じイベントの`matches.json`内で、2つのレコードが同じ`set_id`を
  共有することは無い。
- FR-010: イベントが「完了」（`archive_status: "completed"`付きの
...
新規フィールドは無い。挙動の変更点: `attr.json`の存在が、FR-010に従い
