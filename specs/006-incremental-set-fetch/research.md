...
## 0. 一括クエリを優先し、失敗した場合のみ逐次取得へフォールバックする

**決定**: 既存の一括ページングクエリ（`event.sets`/`phaseGroup.sets`、既存の
`SETS_PER_PAGE_FALLBACKS`フォールバック梯子込み）は廃止せず、引き続き
デフォルトの取得経路とする。`matches.json`がまだ存在しないイベントに対しては
常にこれを最初に試す。§1〜§4で述べる「set一覧の軽量クエリ＋プレースホルダー
投入＋`set(id:)`による個別取得」という逐次取得の仕組みは、一括クエリが実際に
失敗した場合にのみ発動するフォールバックである。また、一括クエリが失敗して
`matches.json`にプレースホルダーが投入された後（＝`matches.json`は存在するが
`attr.json`がまだ無い状態）、後続の実行では一括クエリを再試行せず、直接
逐次取得（残りのプレースホルダーの詳細取得）に進む。

**根拠**: 実運用のイベントの大多数は一括クエリで問題無く完了しており
（`grand_slum`や`SHIBUYA_DAIRAN`のような数百人規模のイベントはむしろ例外）、
`set_id`自体は一括クエリのレスポンスに既に含まれる`id`フィールドから無償で
得られる（`_SET_NODE_FIELDS`参照）。逐次取得を常時・全イベントに適用すると、
今日1回の一括クエリ（多くの場合1〜数ページ）で済んでいたイベントについても、
set一覧取得クエリ＋setの個数分の個別/バッチ取得という、より多くのリクエストを
発行することになってしまう。ユーザーからの明示的な指摘（「いたずらにクエリ
実行回数を増やしたくない」）を踏まえ、逐次取得は問題が実際に発生した場合の
救済手段に限定し、大多数のイベントでは今日と同じリクエスト数に留める。
同様に、一度一括クエリが失敗したと分かっているイベントに対して、実行のたびに
一括クエリを再試行するのも同じ理由で無駄——`matches.json`が存在するのに
`attr.json`が無い、という状態そのものが「このイベントは逐次取得モードに
入っている」という十分な判定材料になる（既存の`event_files_complete()`/
`tournament_events_complete()`が既に`attr.json`の有無で「未完了」を判定して
いるのと同じシグナルを流用する）。

**検討した代替案**:
- イベントの規模（例: `standings`から分かるentrant数）に応じて、一括クエリを
  試す前に逐次取得モードへ直行するかどうかを事前に判断する。却下（今回は
  採用しない）: entrant数としきい値の関係は経験則でしかなく、しきい値の
  選定自体が恣意的になる。まずは「実際に失敗したら切り替える」という単純で
  正確な判定に倣い、事前予測による最適化は将来の改善候補として保留する。
- 常に逐次取得モードを使う（本ドキュメントの当初案）。却下: 上記の通り、
  大多数の小〜中規模イベントでリクエスト数が今日より増えてしまい、
  ユーザーの要求（無駄なクエリ増加を避ける）に反する。
...
- `standings`/`seeds`のエントラント数からset数を推定する。却下: エントラント数
  からは個々の`set_id`が得られず、プレースホルダーの投入（FR-003）や未取得分の
  検出（FR-008）に必要な情報が不足する。
...
（FR-003。ただし一括クエリが失敗した場合にのみ発動するフォールバック——§0参照）
であり、1つのリクエストが二度と「イベント全体」を見る必要が無くなる。
...
`attr.json`が無い）を見つけ出すためのものである——この性質のおかげで、FR-012の
バックフィルは、旧バグにより不完全なまま放置されたイベントが、一括優先・失敗時
のみ逐次取得の経路で再処理された際に自然に拾われる。
...
`data_gap_check.yml`の`skip_report_path`/`_record_skip`および
「Create large-event-skip issue」ステップ——つまり、set取得が
`MaxPagesExceededError`（またはcomplexity超過）で失敗した場合に、それを
「人間が気づいてissueを見て手動でワークフローを再実行すべき事象」として記録・
通知する仕組み一式——を削除する。set取得の一括`event.sets`/`phaseGroup.sets`
ページング自体、および`MaxPagesExceededError`が発生し得ること自体は変わらない
（§0参照。一括クエリはこれまで通り最初に試みられ、失敗もし得る）。変わるのは
その失敗の「扱い」であり、`_record_skip`によるissue化ではなく、
§0〜§3の逐次取得モードへの自動フォールバックに置き換わる。

**根拠**: FR-013/014（spec.md）——`/speckit-clarify`の中でユーザーが見落としでは
...
いた安全網ではなく、実運用上は死んでいた経路であり、かつその失敗そのものは
今後も起こり得るが、以後は人手を介さず自動的に処理される。
...
（FR-010）ため、逐次的に埋められている途中のイベントは今日のバリデータからは
